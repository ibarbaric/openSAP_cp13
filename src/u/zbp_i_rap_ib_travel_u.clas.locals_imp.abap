...
*   booking creation as association from Travel
...
CLASS lhc_Travel IMPLEMENTATION.
  ...
  METHOD create.
    ...
    LOOP AT entities ASSIGNING FIELD-SYMBOL(<entity>).
      ...
      CALL FUNCTION '/DMO/FLIGHT_TRAVEL_CREATE' "does NOT persist to DB - only buffer in FUGR, persist in LSC_*->SAVE method
        EXPORTING
  ...
  METHOD update.
*   changes internal buffer, not DB
    DATA legacy_entity_in   TYPE /dmo/travel.
    ...
    LOOP AT entities ASSIGNING FIELD-SYMBOL(<entity>).
      ...
      legacy_entity_x-_intx = CORRESPONDING zsrap_ib_travel_x( <entity> MAPPING FROM ENTITY ).
...
* read from buffer, not DB
  ...
  METHOD lock.
*   executes on 'Save', before update
...
CLASS lsc_zi_rap_ib_travel_u DEFINITION INHERITING FROM cl_abap_behavior_saver.
  PROTECTED SECTION.
...
CLASS lsc_zi_rap_ib_travel_u IMPLEMENTATION.
  ...
  METHOD save.
    CALL FUNCTION '/DMO/FLIGHT_TRAVEL_SAVE'. "persisting to DB buffered data
  ENDMETHOD.
