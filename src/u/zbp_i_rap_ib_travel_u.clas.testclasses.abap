...
CLASS ltcl_integration_test IMPLEMENTATION.
  ...
  METHOD create_travel.
    ...
    DATA travels_in TYPE TABLE FOR CREATE zi_rap_ib_travel_U\\Travel.
    ...
    MODIFY ENTITIES OF zi_rap_ib_travel_u
      ENTITY travel
    ...
    SELECT * FROM zi_rap_ib_travel_u WHERE TravelId = @new_travel_id INTO TABLE @DATA(lt_travel)  .
  ...
  METHOD class_setup.
    cds_test_environment = cl_cds_test_environment=>create_for_multiple_cds(
        i_for_entities = VALUE #( ( i_for_entity = 'zi_rap_ib_travel_u'  )
                                  ( i_for_entity = 'zi_rap_ib_booking_u' )
                                )
    ).
  ENDMETHOD.
